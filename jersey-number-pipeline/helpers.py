...
def process_entry(entry, all_legible_set, crops_destination_dir):
    """各エントリーの画像を処理する"""
    filtered_points = get_points(entry)
    img_name = entry["img_name"]

    if os.path.basename(img_name) not in all_legible_set:
        return None, None
    if len(filtered_points) == 0:
        return os.path.basename(img_name).split('_')[0], None
    ...
    x1 = int(max(0, x_min))
    y1 = int(max(0, y_min))
    x2 = int(min(width - 1, x_max))
    y2 = int(min(height - 1, y_max))
    ...
    name = os.path.basename(img_name)
    save_path = os.path.join(crops_destination_dir, name)
    cv2.imwrite(save_path, crop)

    return None, img_name  # skipped, saved

def generate_crops_for_all_videos(json_file, crops_destination_dir, legible_results, all_legible=None):
    """すべての動画についてクロップ画像を生成する（並列処理対応）"""
    if all_legible is None:
        all_legible = []
        for video in legible_results.keys():
            for track in legible_results[video].keys():
                for entry in legible_results[video][track]:
                    all_legible.append(os.path.basename(entry))

    all_legible_set = set(all_legible)  # 高速化のため set に変換

    with open(json_file, 'r') as f:
        all_poses = json.load(f)["pose_results"]  # JSON をロード
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        results = list(tqdm(pool.starmap(process_entry, [(entry, all_legible_set, crops_destination_dir) for entry in all_poses]), total=len(all_poses)))

    # 結果を集約
    for skipped, saved in results:
        if skipped:
            skipped_dict[skipped] = skipped_dict.get(skipped, 0) + 1
        if saved:
            saved_list.append(saved)

    print(f"skipped {len(skipped_dict)} categories, total skipped {sum(skipped_dict.values())} out of {len(all_poses)}")
    return skipped_dict, saved_list
...
def process_jersey_id_predictions_outdoor(file_path, useBias=False):
    """
    Outdoor用のファイル名を解析し、動画名とトラックレット名を分割してネストした辞書形式で結果を生成する。
    ファイル名例: "IMG_0104_1_frame0_track1_cropped.png"
    → 動画ID: "IMG_0104_1", トラックレットID: "track1"
    """
    nested_results = {}  # { video: { track: [ [label, confidence], ... ] } }
    final_results = {}   # { video: { track: label, ... } }
    ...
    for name in results_dict.keys():
        ...
        if len(parts) < 5:
            continue
        # 動画IDは最初の3要素を結合（例: "IMG_0104_1"）
        video = '_'.join(parts[:3])
        # トラックレットIDは5番目の要素（例: "track1"）
        track = parts[4]
        # ネストしたキーとして扱う
        if video not in nested_results:
            nested_results[video] = {}
        if track not in nested_results[video]:
            nested_results[video][track] = []
        value = results_dict[name]['label']
        if not is_valid_number(value):
            continue
        confidence = results_dict[name]['confidence']
        total_prob = 1.0
        # 最後の確率は 'end' トークンに対応しているため除外
        for x in confidence[:-1]:
            total_prob *= float(x)
        nested_results[video][track].append([int(value), total_prob])
    
    final_full_results = {}
    for video in nested_results.keys():
        if video not in final_results:
            final_results[video] = {}
            final_full_results[video] = {}
        for track in nested_results[video].keys():
            results = np.array(nested_results[video][track])
            if results.size == 0:
                continue
            best_prediction, all_unique, weights = find_best_prediction(results, useBias=useBias)
            final_results[video][track] = str(int(best_prediction))
            final_full_results[video][track] = {
